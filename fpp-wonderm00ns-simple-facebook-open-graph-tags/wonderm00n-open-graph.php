...
function webdados_fb_open_graph() {
	...
	if (is_singular()) {
		...
		if (intval($fb_image_show)==1 || intval($fb_image_show_schema)==1 || intval($fb_image_show_twitter)==1) {
			$fb_image=webdados_fb_open_graph_post_image($fb_image_use_specific, $fb_image_use_featured, $fb_image_use_cover, $fb_image_use_content, $fb_image_use_media, $fb_image_use_default, $fb_image);
		}
		//Author
...
//Post image
function webdados_fb_open_graph_post_image($fb_image_use_specific=1,$fb_image_use_featured=1, $fb_image_use_cover=1, $fb_image_use_content=1, $fb_image_use_media=1, $fb_image_use_default=1, $default_image='') {
	...
	//Featured cover
	if (!$thumbdone) {
		if (intval($fb_image_use_cover)==1) {
			$cov = get_field('cover_image',$post->ID);
			$cov_wide = get_field('cover_image_wide',$post->ID);
			if ($cov_wide) {
				$fb_image = $cov_wide['size']['medium'];
				$thumbdone = true;
			}
			else if ($cov) {
				$fb_image = $cov['size']['medium'];
				$thumbdone = true;
			}
		}
	}
